 ...
 * @returns
 */
ClavaJoinPoints.cxxConstructExpr = function(type) {
	constructorArguments = arrayFromArgs(arguments, 1);
	constructorArguments = constructorArguments.map((arg) => {
		if (isString(arg)) {
			return CustomClavaJoinPoints.exprLiteral(arg);
 ...
 */
ClavaJoinPoints.varRef = function(decl, $type) {
	if(isString(decl)) {
		//Check.isString(decl);
...
ClavaJoinPoints.functionType = function($returnType) {
	...
	return AstFactory.functionType($returnType, argTypes);
}
 ...
 * 
 */
ClavaJoinPoints.exprStmt = function($expr) {
 ...
 * @return {class}
 */
ClavaJoinPoints.declStmt = function() {
	
	var decls = arrayFromArgs(arguments, 0);
	for(var $decl of decls) {
		Check.isJoinPoint($decl, "decl");	
	}
			
	return AstFactory.declStmt(decls);
}
 ...
 * 
 */
ClavaJoinPoints.comment = function(text) {
